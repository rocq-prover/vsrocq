...
const accordion: FunctionComponent<AccordionProps> = (props) => {
    ...
    const toggleCollapse = collapseHandler
        ? collapseHandler
        : () => setCollapsed(!collapsed);

    const secondaryActionButton =
        seconaryActionHandler !== undefined &&
        seconaryActionIcon !== undefined ? (
            <VSCodeButton
                appearance={"icon"}
                onClick={seconaryActionHandler}
                className={classes.Demphasize}
            >
                {" "}
                {seconaryActionIcon}{" "}
            </VSCodeButton>
        ...
        <div>
            ...
            >
                <VscChevronDown
                    className={
                        !collapsed
                            ? [classes.Rotated, classes.Demphasize].join(" ")
                            : ""
                    }
                />
            </VSCodeButton>
    ...
    return (
        <div className={classes.Block}>
            ...
            <div className={classes.Header}>
                <span className={!collapsed ? classes.Demphasize : ""}>
                    {title}
                </span>
                {actionRow}
            </div>
            ...
            <div className={panelClasses.join(" ")}>{props.children}</div>
        </div>
    );
};
