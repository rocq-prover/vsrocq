...
let log = ParTactic.TacticWorkerProcess.log 

let main_worker options ~opts:_ state =
  let initial_vernac_state = Vernacstate.freeze_full_state () in
  try ParTactic.TacticWorkerProcess.main ~st:initial_vernac_state options
  with exn ->
...
let () =
  Coqinit.init_ocaml ();
  let opts, emoptions = Coqinit.parse_arguments ~parse_extra:ParTactic.TacticWorkerProcess.parse_options ~usage:vsrocqtop_specific_usage () in
...
let () =
  Coqinit.init_ocaml ();
  let opts, emoptions = Coqinit.parse_arguments ~parse_extra:ParTactic.TacticWorkerProcess.parse_options (List.tl (Array.to_list Sys.argv)) in
