...
let rec handle_dm_events n (events : DocumentManager.event Sel.Todo.t) st =
  if n <= 0 then (Stdlib.Format.eprintf "handle_dm_events run out of steps:\nTodo = %a\n" (Sel.Todo.pp DocumentManager.pp_event) events; Stdlib.exit 1)
    ...
    | Some ev ->
      ...
      let st, new_events =
        match DocumentManager.handle_event ev st ~block:false Protocol.Settings.Mode.Manual Protocol.Settings.Goals.Diff.Mode.Off Protocol.Settings.Goals.PrettyPrint.Pp ~showOnlyPropHypotheses:false with
