...
let empty_analysis root_library =
  {
...
let add_graph_node graph node =
  if StringMap.mem node graph then graph
  else StringMap.add node StringSet.empty graph
...
let local_node_id ident = "local:" ^ Ident.unique_name ident
let external_node_id display_name = "external:" ^ display_name
...
let add_external_node analysis display_name =
  ...
  let display_names =
    ensure_display_name analysis.display_names node display_name
  in
...
let add_local_binding analysis prefix ident =
  ...
  in
  let latest_bindings =
    StringMap.add display_name node analysis.latest_bindings
  in
  let ident_bindings = Ident.add ident node analysis.ident_bindings in
  { analysis with graph; local_nodes; display_names; latest_bindings; ident_bindings; }, node
...
let binding_for_display_name analysis display_name =
  ...
  | None ->
      if
        analysis.root_library = ""
        || has_prefix (analysis.root_library ^ ".") display_name
      then None
      else
        StringMap.find_opt
          (analysis.root_library ^ "." ^ display_name)
          analysis.latest_bindings

let resolve_path analysis path =
  let display_name = Path.name path in
  match Path.flatten path with
  | `Ok (head, []) -> begin
      match Ident.find_same head analysis.ident_bindings with
      | node -> (analysis, node)
      | exception Not_found -> begin
          match binding_for_display_name analysis display_name with
...
let resolve_path_for_source analysis source path =
  let unresolved_display_name = Path.name path in
  match Path.flatten path with
  | `Ok (head, []) -> begin
      match Ident.find_same head analysis.ident_bindings with
      | node -> (analysis, node)
      | exception Not_found -> begin
          match binding_for_display_name analysis unresolved_display_name with
          | Some node -> (analysis, node)
          | None ->
              ...
              in
              let display_name =
                source_display_name ^ "." ^ unresolved_display_name
              in
...
let callsite_node_id callee_target loc =
  ...
  Printf.sprintf "callsite:%s:%d:%d:%d:%d" callee_target start.Lexing.pos_lnum
...
let add_callsite_node analysis callee_target loc =
  ...
  in
  let display_names =
    ensure_display_name analysis.display_names node callee_display_name
  in
...
let expression_is_function exp = type_is_function exp.exp_type

[%%if ocaml_version < (5, 0, 0)]

let function_argument_cases exp =
  match exp.exp_desc with Texp_match (_, cases, _) -> Some cases | _ -> None
...
let rec iter_function_calls analysis source expression =
  ...
  let record_call current_source path =
    let analysis, target =
      resolve_path_for_source !analysis_ref current_source path
    in
  ...
  let callback_source callee_target loc =
    let analysis, callsite_node =
      add_callsite_node !analysis_ref callee_target loc
    in
  ...
  let rec record_function_argument_call current_source exp =
    match exp.exp_desc with
    | Texp_ident (path, _, _) -> ignore (record_call current_source path)
    | Texp_let (_, _, body) | Texp_sequence (_, body) ->
    ...
    | Texp_function _ | Texp_apply _ -> ()
    | _ -> begin
        match function_argument_cases exp with
        | Some cases ->
            List.iter
              (fun case ->
                record_function_argument_call current_source case.c_rhs)
              cases
        | None -> ()
      end
  in
              ...
              iter_apply_arguments args (fun argument ->
                  if expression_is_function argument then begin
                    let thunk_source =
                      ...
                      | Some callee_target ->
                          callback_source callee_target exp.exp_loc
                      | None -> source
                    in
                    record_function_argument_call thunk_source argument;
                    analysis_ref :=
                      iter_function_calls !analysis_ref thunk_source argument
                  end
...
let bound_idents pattern = Typedtree.pat_bound_idents pattern

[%%if ocaml_version < (5, 0, 0)]

let collect_module_expr_desc collect_structure collect_module_expr prefix
    analysis = function
  ...
  | Tmod_constraint (module_expr, _, _, _) ->
      collect_module_expr prefix analysis module_expr
...
and collect_structure_item prefix analysis item =
  ...
  | Tstr_module module_binding -> begin
      match module_name module_binding with
      | Some name ->
          collect_module_expr (prefix @ [ name ]) analysis
            module_binding.mb_expr
      | None -> analysis
    end
...
and collect_value_binding prefix analysis binding =
  ...
  | _ ->
      ...
      List.fold_left
        (fun analysis source ->
          iter_function_calls analysis source binding.vb_expr)
        analysis sources

and collect_module_expr prefix analysis module_expr =
  collect_module_expr_desc collect_structure collect_module_expr prefix analysis
    module_expr.mod_desc

let maybe_read_cmt file_path =
  try Some (read_cmt file_path) with Error _ -> None
...
let load_workspace_graph input_cmt_path =
  ...
  let initial_analysis = empty_analysis root_library in
  ...
  in
  let analysis =
    List.fold_left collect_file initial_analysis
      (cmt_files_in_directory directory)
  in
...
let roots_for_prefix root_prefix analysis =
  StringSet.filter
    (fun node ->
      starts_with_parts root_prefix
        (String.split_on_char '.' (display_name analysis node)))
    ...
    (fun callee chains ->
      let callee =
        if StringSet.mem callee analysis.local_nodes then callee
          ...
          match
            binding_for_display_name analysis (display_name analysis callee)
          with
      ...
      if StringSet.mem callee analysis.local_nodes then
        if StringSet.mem callee visited then chains
          ...
          collect_chains analysis
            (StringSet.add callee visited)
            next_path callee chains
...
let compile_component_regexp pattern = Str.regexp ("^" ^ pattern)
...
let strip_root_prefix root_prefix display_name =
  let root = String.concat "." root_prefix in
  let root_with_dot = root ^ "." in
  if is_prefix root_with_dot display_name then
    String.sub display_name
      (String.length root_with_dot)
      (String.length display_name - String.length root_with_dot)
  else display_name
...
let display_chain analysis root_prefix chain =
  chain
  |> List.map (fun node ->
      strip_root_prefix root_prefix (display_name analysis node))
  |> compress_display_chain

let matches_regexp regexp entry = Str.string_match regexp entry 0
    ...
    (fun pattern ->
      ...
      if pattern_length > chain_length then false
        ...
        let rec drop count items =
          ...
          else match items with [] -> [] | _ :: tail -> drop (count - 1) tail
        in
...
let safe_barrier_index safe_barriers rendered_chain =
  let rec loop index = function
    | [] -> None
    | entry :: tail ->
        if List.exists (fun regexp -> matches_regexp regexp entry) safe_barriers
        then Some index
...
let truncate_at_safe_barrier safe_barriers rendered_chain =
  ...
  | Some barrier_index ->
      let keep_until_index =
        if
          List.length rendered_chain > barrier_index + 1
          && List.nth rendered_chain (barrier_index + 1) = "[thunk]"
        then barrier_index + 1
        else barrier_index
      in
      let prefix, _suffix =
        List.fold_left
    ...
    StringSet.filter
      (fun rendered_line ->
        if string_ends_with ~suffix:" -> [thunk] -> *" rendered_line then true
          ...
          let prefix_length =
            String.length rendered_line - String.length " -> *"
          in
...
let usage () =
  prerr_endline
    "usage: static_callgraph [--whitelist file.txt] [--safe-barriers file.txt] \
     file.cmt";
  exit 2
