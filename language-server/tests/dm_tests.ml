...
let edit_text st ~start ~stop ~text =
  ...
  let st, events = Bridge.apply_text_edits st [(range, text)] in
  let todo = Sel.Todo.add Sel.Todo.empty events in
  handle_dm_events todo st

  let apply_text_edits st l =
    let st, events = Bridge.apply_text_edits st l in
    let todo = Sel.Todo.add Sel.Todo.empty events in
    handle_dm_events todo st
...
let%test_unit "parse.init" =
  ...
  let doc = Document.raw_document @@ Bridge.Internal.document st in
  [%test_eq: int] (RawDocument.end_loc doc) 44;
  let sentences = Document.sentences @@ Bridge.Internal.document st in
...
let%test_unit "parse.invalidate_before_module" =
  ...
  let st, events = Bridge.interpret_to_end st Settings.Mode.Manual in
...
let%test_unit "exec.init" =
  ...
  let todo = Sel.Todo.(add init_events events) in
  let st = handle_dm_events todo st in
  let ranges = (Bridge.executed_ranges st Settings.Mode.Manual).processed in
...
let%test_unit "step_forward.proof_view" =
  ...
  let st = handle_dm_events todo st in
  [%test_pred: sentence_id option] (Option.equal State.Id.equal (Some s2.id)) (Bridge.Internal.observe_id st);
  let data = Bridge.get_proof st Host.Settings.Goals.Diff.Mode.Off None in
...
let%test_unit "step_backward.document_begin" =
  ...
  let st = handle_dm_events todo st in
  let todo = Sel.Todo.(add init_events events) in
  let st, events = Bridge.interpret_to_previous st Settings.Mode.Manual in
  let st = handle_dm_events todo st in
  [%test_eq: bool] (Option.is_none (Bridge.Internal.observe_id st)) true
...
let%test_unit "exec.insert" =
  ...
  let st = Bridge.validate_document st in
  let st, events = Bridge.interpret_to_end st in
  let st = insert_text st ~loc:0 ~text:"Definition z := 0. " in
  let st = Bridge.validate_document st in
  let st, events = Bridge.interpret_to_end st in
  let ranges = (Bridge.executed_ranges st).processed in
...
let%test_unit "edit.shift_warning_in_sentence" =
  ...
  let warning = Stdlib.List.hd @@ Bridge.all_diagnostics st in
