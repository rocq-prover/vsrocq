...
let%test_unit "goals: encoding after replay from top" =
  ...
  let st, exec_events = Bridge.interpret_to_next st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
  let st, exec_events = Bridge.interpret_to_previous st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
  let st, exec_events = Bridge.interpret_to_next st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
  let proof = Stdlib.Option.get (Bridge.get_proof st Protocol.Settings.Goals.Diff.Mode.Off None) in
...
let%test_unit "goals: proof is available after error" =
  ...
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
  let proof = Bridge.get_proof st Protocol.Settings.Goals.Diff.Mode.Off None in
