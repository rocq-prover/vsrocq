...
let%test_unit "goals: encoding after replay from top" =
  ...
  let st, exec_events = DocumentManager.interpret_to_next (`String "test") st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
  let st, exec_events = DocumentManager.interpret_to_previous (`String "test") st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
  let st, exec_events = DocumentManager.interpret_to_next (`String "test") st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  ...
  let _json = Protocol.ExtProtocol.Notification.Server.ProofViewParams.yojson_of_t { proof = Some proof; messages; pp_proof = None; pp_messages = []; range = Range.top(); request_id = None } in
  ()
...
let%test_unit "goals: proof is available after error" =
  ...
  let st, exec_events = DocumentManager.interpret_to_end (`String "test") st Settings.Mode.Manual in
  let todo = Sel.Todo.(add empty exec_events) in
  let st = handle_dm_events todo st in
