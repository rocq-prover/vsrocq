...
let trdeps doc s1 =
  ...
  let rec trclose f s =
    let s' = State.Id.Set.fold (fun x acc -> State.Id.Set.union (f x) acc) s s in
    if State.Id.Set.equal s s' then s
    else trclose f s' in
  trclose (Scheduler.dependents (Document.schedule doc)) (State.Id.Set.singleton s1.id)
...
let%test_unit "parse.validate_errors_twice" = 
  ...
  [%test_eq: sentence_id list] [] (State.Id.Set.elements invalid_ids);
...
let %test_unit "document: invalidate top" =
  ...
  [%test_eq: sentence_id list] [s1.id] (State.Id.Set.elements invalid_ids);
  ...
  [%test_pred: sentence_id] (fun x -> State.Id.Set.mem x deps) r2.id;
  [%test_pred: sentence_id] (fun x -> State.Id.Set.mem x deps) r3.id;
  [%test_pred: sentence_id] (fun x -> State.Id.Set.mem x deps) r4.id;
  [%test_pred: sentence_id] (fun x -> State.Id.Set.mem x deps) r5.id;
  ()
...
let %test_unit "document: invalidate proof" =
  ...
  let Document.{unchanged_id; invalid_ids; parsed_document} = validate_document parsed_document in
  let r1,(r2,(r3,(r4,(r5,())))) = d_sentences parsed_document (P(P(P(P(P(O)))))) in
  [%test_eq: sentence_id list] [s3.id] (State.Id.Set.elements invalid_ids);
  ...
  [%test_pred: sentence_id] (fun x -> not (State.Id.Set.mem x deps)) r1.id;
  [%test_pred: sentence_id] (fun x -> not (State.Id.Set.mem x deps)) r2.id;
  [%test_pred: sentence_id] (fun x -> not (State.Id.Set.mem x deps)) r3.id;
  [%test_pred: sentence_id] (fun x -> not (State.Id.Set.mem x deps)) r5.id;
  ()
...
let %test_unit "document: invalidate proof 2" =
  ...
  [%test_eq: sentence_id list] [s3.id;s4.id;s5.id] (State.Id.Set.elements invalid_ids);
  [%test_eq: sentence_id option] (Some s2.id) unchanged_id;
  [%test_eq: sentence_id] s1.id r1.id;
  [%test_eq: sentence_id] s2.id r2.id
...
let %test_unit "document: delete line" = 
  ...
  let Document.{unchanged_id; invalid_ids; parsed_document} = validate_document parsed_document in 
  let sentences_id = State.Id.Set.of_list (List.map (Document.sentences parsed_document) ~f:(fun s -> s.id)) in
  [%test_pred: sentence_id] (fun x -> not (State.Id.Set.mem x sentences_id)) s5.id;
  ()
