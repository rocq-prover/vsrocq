...
let check ~doc_id ~vs ~pattern =
  ProverThread.run ~doc_id ~name:"check" ~timeout (fun () -> check ~vs ~pattern) |>
  to_types_error

let jump_to_definition ~doc_id ~vs opattern =
  ProverThread.run ~doc_id ~name:"jump_to_definition" ~timeout (fun () -> jump_to_definition ~vs opattern) |>
  to_option |> Option.flatten

let locate ~doc_id ~vs ~pattern =
  ProverThread.run ~doc_id ~name:"locate" ~timeout (fun () -> locate ~vs ~pattern) |>
  to_types_error

let search ~doc_id ~vs ~id pattern =
  ProverThread.run ~doc_id ~name:"search" ~timeout:0.5 (fun () -> search ~vs ~id pattern) |>
  to_list

let print ~doc_id ~vs ~pattern = 
  ProverThread.run ~doc_id ~name:"print" ~timeout (fun () -> print ~vs ~pattern) |>
  to_types_error

let hover document pos =
  ProverThread.run ~doc_id:(Document.id document) ~name:"hover" ~timeout
    (fun () -> hover document pos) |>
  to_option |> Option.flatten

let about ~doc_id ~vs ~pattern =
  ProverThread.run ~doc_id ~name:"about" ~timeout (fun () -> about vs ~pattern) |>
  to_types_error

let get_completions ~doc_id ~vs =
  ProverThread.run ~doc_id ~name:"get_completions" ~timeout:0.5 (fun () -> get_completions ~vs) |>
  to_list
