...
type document = {
  ...
  schedule : Host_common.Scheduler.schedule;
  outline : outline;
  parsed_loc : int;
...
let create_parsing_event event =
  let priority = Some Host_common.PriorityManager.parsing in
...
let add_sentence parsed parsing_start start stop (ast: sentence_state) synterp_state scheduler_state_before =
  ...
  let scheduler_state_after, schedule = 
    match ast with
    | Error {msg} ->
      scheduler_state_before, Host_common.Scheduler.schedule_errored_sentence id msg parsed.schedule
    | Parsed ast ->
      let ast' = (ast.ast, ast.classification, synterp_state) in
      Host_common.Scheduler.schedule_sentence (id, ast') scheduler_state_before parsed.schedule
  in
...
let state_after_sentence parsed = function
  ...
  | None -> (-1, parsed.init_synterp_state, Host_common.Scheduler.initial_state)
